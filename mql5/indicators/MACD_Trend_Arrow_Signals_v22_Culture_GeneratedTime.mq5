...
// CultureCapital Generated-Time research stream.
// Legacy 15-column CSV remains unchanged.
string CultureGeneratedCSVFileName()
{
   return(CSV_FilePrefix+"_CultureCapital_GeneratedTime.csv");
}

bool CultureGeneratedTimeIsValid(datetime generated_time)
{
   if(BrokerTag()!="CultureCapital")
      return(false);
   ...
   if(StringFind(probe,"culturecapital")<0 && StringFind(probe,"culture capital")<0)
   {
      Print("GeneratedTime blocked: unexpected Culture server=",server);
      return(false);
   }

   datetime gmt_time=TimeGMT();
   long diff=(long)generated_time-(long)gmt_time;
   if(diff<0) diff=-diff;

   // Culture expected server basis = UTC+0.
   // Fail closed for the new research stream if deviation exceeds 120 seconds.
   if(diff>120)
   {
      Print("GeneratedTime blocked: UTC validation failed. server=",server,
            " TimeCurrent=",TimeToString(generated_time,TIME_DATE|TIME_SECONDS),
            " TimeGMT=",TimeToString(gmt_time,TIME_DATE|TIME_SECONDS),
            " diff_sec=",diff);
      return(false);
   }
...
string FormatGeneratedUTC(datetime value)
{
   MqlDateTime t;
...
void LogCultureGeneratedSignal(datetime bar_time,string direction,int score,string signal_class,double price,
                               bool ma_ok,bool ichi_ok,bool cloud_ok,bool momentum_ok,bool atr_ok,double ratio)
{
   if(!Enable_CSV_Log) return;
   if(BrokerTag()!="CultureCapital") return;

   datetime generated_time=TimeCurrent();

   if(!CultureGeneratedTimeIsValid(generated_time))
      return;

   string file_name=CultureGeneratedCSVFileName();
   int h=FileOpen(file_name,FILE_READ|FILE_WRITE|FILE_CSV|FILE_COMMON|FILE_ANSI,';');

   if(h==INVALID_HANDLE)
   {
      Print("GeneratedTime CSV open error: ",GetLastError()," file=",file_name);
      return;
   }

   string bar_time_text=TimeToString(bar_time,TIME_DATE|TIME_MINUTES);
   string broker_text=BrokerTag();
   string symbol_text=_Symbol;
   string period_text=PeriodText();

   // ------------------------------------------------------------
   // Permanent duplicate protection
   // Key = BROKER + SYMBOL + PERIOD + BAR_OPEN_TIME
   // Prevents duplicate rows after indicator reload / MT5 restart.
   // ------------------------------------------------------------
   ...
   while(!FileIsEnding(h))
   {
      ...
      string f_broker     =FileReadString(h);
      string f_server     =FileReadString(h);
      string f_symbol     =FileReadString(h);
      string f_period     =FileReadString(h);
      string f_direction  =FileReadString(h);
      string f_score      =FileReadString(h);
      string f_class      =FileReadString(h);
      string f_price      =FileReadString(h);
      string f_ma         =FileReadString(h);
      string f_ichi       =FileReadString(h);
      string f_cloud      =FileReadString(h);
      string f_momentum   =FileReadString(h);
      string f_macd_atr   =FileReadString(h);
      string f_ratio      =FileReadString(h);
      string f_generated  =FileReadString(h);

      // Header is automatically ignored because TIME/BROKER/etc.
      // cannot equal the actual signal key.
      if(f_time   ==bar_time_text &&
         f_broker ==broker_text   &&
         f_symbol ==symbol_text   &&
         f_period ==period_text)
      {
         duplicate=true;
         break;
      }
   ...
   {
      FileClose(h);
      Print("GeneratedTime duplicate blocked: ",
            broker_text," ",
            symbol_text," ",
            period_text," ",
            bar_time_text);
   ...
   // New file: write the 16-column header.
   if(FileSize(h)==0)
   {
      FileWrite(h,
                "TIME",
   ...
   FileWrite(h,
             ...
             (int)momentum_ok,
             (int)atr_ok,
             DoubleToString(ratio,6),
             FormatGeneratedUTC(generated_time));
 ...
 const long &tick_volume[],const long &volume[],const int &spread[])
{
   ...
   for(int i=available-2;i>=1;i--)
   {
      ...
      if(i==1 && time[i]!=LastLoggedBarTime)
      {
         ...
         // LogSignal(time[i],direction,score,cls,signal_price,ma_ok,ichi_ok,cloud_ok,momentum_ok,MACD_StrongEnough);
         LogCultureGeneratedSignal(time[i],direction,score,cls,signal_price,ma_ok,ichi_ok,cloud_ok,momentum_ok,MACD_StrongEnough,ratio);
